...
fetch(
  ...
  .then((data) => {
    ...
    const dataset = data.daily.map((obj) =>
      Math.round((obj.temp.min + obj.temp.max) / 2)
    );
    dataset.forEach((element, index) => {
      ...
      htmlDaLista =
        htmlDaLista +
        '<div class="tempGeral" style="border:0.5rem solid' +
        cor +
        '">' +
        '<div class="classTemp">' +
        element +
        'º</div><br><div class="classDate">' +
        diasTimeStamp[index].substring(5, -1) +
        "</div></div>";
    });
